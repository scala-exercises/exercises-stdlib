...
object InfixTypes extends AnyFlatSpec with Matchers with org.scalaexercises.definitions.Section {
   ...
   *
   * The infix operator `op` may be an arbitrary identifier, except for `*`, which is reserved as a
   * postfix modifier denoting a repeated parameter type.
   *
   * We can make a type infix, meaning that the type can be displayed in complement between two
   * types in order to make a readable declaration:
   */
  ...
  /**
   * Of course we can make this a bit more elegant by creating an infix operator method to use with
   * our infix type:
   */
