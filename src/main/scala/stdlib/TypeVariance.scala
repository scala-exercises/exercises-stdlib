...
object TypeVariance extends AnyFlatSpec with Matchers with org.scalaexercises.definitions.Section {
  ...
  /**
   * A traditional objection to static typing is that it has much syntactic overhead. Scala
   * alleviates this by providing type inference.
   *
  ...
  /**
   * Scala's type system has to account for class hierarchies together with polymorphism. Class
   * hierarchies allow the expression of subtype relationships. A central question that comes up
   * when mixing OO with polymorphism is: if `T'` is a subclass of `T`, is `Container[T']`
   * considered a subclass of `Container[T]`? Variance annotations allow you to express the
   * following relationships between class hierarchies & polymorphic types:
   *
   * ==Covariant==
   ...
   *   - Scala notation: `[T]`
   *
   * That one probably blew your mind. Now if you assign a type to the instantiation that is
   * different to the variable type, you'll have problems. You may want to take time after this koan
   * to compare and contrast with the previous one.
   *
   * {{{
   * class MyContainer[A](val a: A)(implicit manifest: scala.reflect.Manifest[A]) {
   * def contents = manifest.runtimeClass.getSimpleName
   ...
   * This will allow you to set the container to either a variable with the same type or parent
   * type. In other words, you can assign `MyContainer[Fruit]` or `MyContainer[Citrus]`.
   */
  ...
  /**
   * The problem with covariance is that you can't mutate, set or change the object since it has to
   * guarantee that what you put into it is a valid type. In other words the reference is a fruit
   * basket, but we still have to make sure that no other fruit can be placed in our orange basket:
   */
  ...
  /**
   * Declaring `-` indicates contravariance variance. Using `-` you can apply any container with a
   * certain type to a container with a superclass of that type. This is reverse to covariant. In
   * our example, we can set a citrus basket to an orange or tangelo basket. Since an orange or
  ...
  /**
   * Declaring neither `-` nor `+` indicates invariance. You cannot use a superclass variable
   * reference (contravariant position) or a subclass variable reference (covariant position) of
   * that type. In our example, this means that if you create a citrus basket you can only reference
